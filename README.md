...
<!-- LATEST:START -->
_داده هنوز جمع‌آوری نشده — بعد از اولین اجرای Action این بخش خودکار پر می‌شه._
<!-- LATEST:END -->
...
- اگه ساختار صفحه‌ی tgju تغییر کنه، اسکریپت ممکنه از کار بیفته تا کسی اصلاحش کنه.
- درخواست‌ها با فاصله‌ی زمانی و فقط یک‌بار در روز زده می‌شن؛ فشاری به سرور اونا وارد نمی‌کنه.
...
## راه‌اندازی برای فورک/توسعه‌ی خودت

۱. ریپو رو فورک یا کلون کن.
۲. هر ۴ آدرس توی `scripts/update.py` (متغیر `PAGES`) رو با curl تست کن و مطمئن شو سلکتور `span.price[data-col="info.last_trade.PDrCotVal"]` مقدار درست برمی‌گردونه:
   ```bash
   curl -s -A "Mozilla/5.0" https://www.tgju.org/profile/geram18 | grep -i 'PDrCotVal'
   ```
...
- اگه HTML سایت تغییر کنه، ممکنه چند روز طول بکشه تا اصلاح بشه (PR خوش‌اومده).

## مشارکت

اگه یه اسلاگ اشتباهه، یا منبع بهتری سراغ داری، یه Issue یا Pull Request باز کن.
